...
int main()
{
  ...
  // open mesh
  int errorCode = vcg::tri::io::Importer<CMesh>::Open(meshA, filenameA.c_str());
  if(errorCode) {
    printf("Error in reading %s: '%s'\n",filenameA ,vcg::tri::io::Importer<CMesh>::ErrorMsg(errorCode));
    if(vcg::tri::io::Importer<CMesh>::ErrorCritical(errorCode)) exit(-1);
  }
